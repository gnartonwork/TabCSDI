...
class tabular_dataset(Dataset):
    ...
    def __init__(
        self, eval_length=66, use_index_list=None, aug_rate=1, missing_ratio=0.1, seed=0
    ):
        self.eval_length = eval_length
        np.random.seed(seed)

        dataset_path = "./data_tbi/tbi_encoded.csv"
        processed_data_path = (
            f"./data_tbi/missing_ratio-{missing_ratio}_seed-{seed}.pk"
        )
        processed_data_path_norm = (
            f"./data_tbi/missing_ratio-{missing_ratio}_seed-{seed}_max-min_norm.pk"
        )
